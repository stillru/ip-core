...
(defun ip-invoice--get-clock-entries (start end client-id)
  ...
  (let ((entries '())
    ...
    (ip-debug-log 'info 'invoice "🔍 [CLOCK] Начало поиска для клиента: %s" client-id)
    (ip-debug-log 'info 'invoice "📅 [CLOCK] Диапазон: %s — %s" start end)
    (let ((tasks-file-path (expand-file-name ip-tasks-file ip-org-directory)))
      (if (not (file-exists-p tasks-file-path))
          (progn
            (ip-debug-log 'error 'invoice "❌ [CLOCK] Файл не существует: %s" tasks-file-path)
            entries)
        (with-current-buffer (find-file-noselect tasks-file-path)
          (ip-debug-log 'info 'invoice "📁 [CLOCK] Открыт буфер: %s" (buffer-name))
          (org-element-map (org-element-parse-buffer) 'headline
            (lambda (headline)
              (let* ((heading (org-element-property :raw-value headline))
                ...
                (ip-debug-log 'debug 'invoice "🔍 [CLOCK] Проверка заголовка: %s" heading)
                (ip-debug-log 'debug 'invoice "🏷️ [CLOCK] CLIENT свойство: %S" client-prop)
                (ip-debug-log 'debug 'invoice "🔖 [CLOCK] Теги: %S" tags)
                (ip-debug-log 'debug 'invoice "✅ [CLOCK] Совпадение по свойству: %s" (if has-client-prop "да" "нет"))
                (ip-debug-log 'debug 'invoice "✅ [CLOCK] Совпадение по тегу: %s" (if has-client-tag "да" "нет"))

                (when (or has-client-prop has-client-tag)
                  (ip-debug-log 'info 'invoice "✅ [CLOCK] Клиент %s найден в: %s" client-id heading)
                  (let ((rate (string-to-number
                               (or (plist-get (ip-get-client-by-id client-id) :DEFAULT_RATE) "0"))))
                    (ip-debug-log 'info 'invoice "💶 [CLOCK] Ставка: %.2f EUR/час" rate)
                      ...
                      (lambda (cl)
                        (when-let ((ts (org-element-property :value cl))
                          ...
                          (let* ((raw-ts (org-element-property :raw-value ts))
                            ...
                            (ip-debug-log 'debug 'invoice "⏱️ [CLOCK] Найдена CLOCK: %s | Длительность: %s" raw-ts duration-str)
                            (when (and (time-less-p start-ts clock-start)
                                       (time-less-p clock-start end-ts))
                              (ip-debug-log 'debug 'invoice "📅 [CLOCK] CLOCK в диапазоне: %s" raw-ts)
                              (let* ((parts (split-string duration-str ":"))
                                ...
                                (ip-debug-log 'info 'invoice "✅ [CLOCK] Добавляем: %s | %.2f ч | %.2f EUR" date hours-float amount)
                                (push (list :date date
                                      ...
                                      entries))))))))))))
          'file))
    (ip-debug-log 'info 'invoice "✅ [CLOCK] Найдено записей: %d" (length entries))
    (dolist (entry entries)
      (ip-debug-log 'info 'invoice "📋 [CLOCK] Итоговая запись: %S" entry))
